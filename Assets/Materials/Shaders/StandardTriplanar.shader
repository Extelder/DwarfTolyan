Shader "Standard Triplanar"
{
    ...
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            tx = IN.localCoord.yz * _EmissionMapScale;
            ty = IN.localCoord.zx * _EmissionMapScale;
            tz = IN.localCoord.xy * _EmissionMapScale;

            half4 ex = tex2D(_EmissionTex, tx) * bf.x;
            half4 ey = tex2D(_EmissionTex, ty) * bf.y;
            half4 ez = tex2D(_EmissionTex, tz) * bf.z;
            half4 Ecolor = (ex + ey + ez) * _EmissionColor;
            o.Emission = Ecolor;
